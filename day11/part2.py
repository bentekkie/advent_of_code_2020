...
f=lambda i,j,k,l,x:p>i>=0and b>j>=0and(x[i][j]=="#"or(x[i][j]=="."and f(i+k,j+l,k,l,x)))
d=lambda i,j,y:sum(f(i+k,j+l,k,l,y)for k in[-1,0,1]for l in[-1,0,1]if(k,l)!=(0,0))
m=lambda i,j,x:"#"if x[i][j]=="L"and d(i,j,x)==0else("L"if x[i][j]=="#"and d(i,j,x)>4 else x[i][j])
